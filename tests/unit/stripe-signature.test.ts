...
function signHeader(payload: string, secret: string, timestamp: number): string {
...
describe('Stripe — verifyStripeSignature', () => {
  ...
  it('accepte un en-tête avec plusieurs signatures v1 (rotation de clés)', () => {
    const ts = nowSeconds();
    const valid = createHmac('sha256', SECRET).update(`${ts}.${payload}`).digest('hex');
    const stale = 'a'.repeat(64);
    expect(verifyStripeSignature(payload, `t=${ts},v1=${stale},v1=${valid}`, SECRET)).toBe(true);
  });
  ...
  it('refuse les en-têtes malformés ou incomplets', () => {
    ...
    expect(verifyStripeSignature(payload, 'no-equals-sign', SECRET)).toBe(false);
    expect(verifyStripeSignature(payload, `t=${ts}`, SECRET)).toBe(false);
    expect(verifyStripeSignature(payload, `v1=${'b'.repeat(64)}`, SECRET)).toBe(false);
    expect(verifyStripeSignature(payload, 't=notanumber,v1=ff', SECRET)).toBe(false);
  });

  it('refuse une signature non hexadécimale sans lever d\'exception', () => {
    const ts = nowSeconds();
    expect(verifyStripeSignature(payload, `t=${ts},v1=zz-invalide-hex`, SECRET)).toBe(false);
  });
