...
export function detectRegion(locale: string, languages: readonly string[]): RegionKey {
  const code = (locale || languages?.[0] || '').toLowerCase();
  if (code.startsWith('fr-ma') || code.startsWith('ary') || code.includes('-ma')) return 'ma';
  ...
  if (code.startsWith('fr')) {
    ...
    if (code.includes('sn') || code.includes('ci') || code.includes('ml') || code.includes('bf') || code.includes('tg') || code.includes('ne') || code.includes('cd')) return 'af';
    return 'eu';
  }
